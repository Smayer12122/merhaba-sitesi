...
//
// Metinleri uretim.js kurar; burada yalnızca adres verilip yanıta sarılır.

function robots(req, res) {
  yanitla(res, 200, req.method === 'HEAD' ? undefined : uretim.robotsMetni(kokAdres(req)), {
...
function siteHaritasi(req, res) {
  yanitla(res, 200, req.method === 'HEAD' ? undefined : uretim.siteHaritasiMetni(kokAdres(req)), {
...
// Panelin giriş penceresi, panelin açıldığı adresten geçmek zorunda: Decap
// gelen mesajın kaynağını base_url ile karşılaştırıyor, tutmazsa mesajı
// yok sayıyor ve panel hiç açılmıyor. Bu yüzden base_url dosyadaki sabit
// değerle değil, isteğin geldiği adresle doldurulur. Böylece panel yerelde de,
// onrender.com adresinde de, özel alan adında da elle ayar gerektirmeden
// çalışır.
function panelAyari(req, res) {
  return fs.promises
    .readFile(path.join(PUBLIC_DIR, 'admin', 'config.yml'), 'utf8')
    .then((metin) => {
      const govde = metin.replace(/^(\s*base_url:).*$/m, `$1 ${kokAdres(req)}`);
...
const server = http.createServer(async (req, res) => {
  ...
  if (yol === '/admin/config.yml') return panelAyari(req, res);
